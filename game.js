...
function onCellClick(r, c) {
    ...
    if (awaitingEliminationFrom) {
        ...
        setMessage('请点击绿色高亮的棋子完成消除。', 'error');
        return;
    }
    ...
    if (!selected) {
        ...
        setMessage('选择一个蓝色空格移动，或点击绿色高亮的同款棋子直接消除。');
        render();
        return;
    }
    ...
    // If clicked another non-empty but not eliminable, show reason
    if (piece !== EMPTY) {
        if (piece !== selPiece) {
            setMessage('这两个格子的内容不同，不能消除。', 'error');
        } else if (!(from.row === r || from.col === c)) {
            setMessage('不在同一行或同一列，不能消除。', 'error');
        } else {
            setMessage('两者之间有阻挡，不能消除。', 'error');
        }
    }
...
function onPointerDown(e, r, c) {
    ...
    // Do not override existing selection on pointer down; this allows second-click elimination
    if (!selected) {
        selected = { row: r, col: c };
        computeValidMoveTargets(selected);
        computeEliminationCandidates(selected);
